...
def main():
    ...
    for planet in swapi_planets:
        # if planet['climate'].lower() == 'tropical' # misses records
        if 'tropical' in planet['climate']:
            tropical.append(planet['name'])
    ...
    tropical.clear()
    tropical = [p['name'] for p in swapi_planets if 'tropical' in p['climate']]
    ...
    planets.clear() # reset
    planets = [utils.filter_data(planet, Planet.KEYS) for planet in swapi_planets]
