...
def main():
    ...
    while jedi.strength > 0 and sith.strength > 0:
        ...
        jedi.wield_lightsaber(None)

        # 1.2.3 Sith wields lightsaber
        # FIX ME = game.random_saber_action() # unpack
        sith.wield_lightsaber(None)

        print(f"\n{jedi.name} {jedi.action} vs {sith.name} {sith.action}")

        # 1.2.4 Score combat round
        # FIX ME = game.score_matrix(jedi.action, sith.action) # unpack
        jedi.change_strength(None)
        sith.change_strength(None)
