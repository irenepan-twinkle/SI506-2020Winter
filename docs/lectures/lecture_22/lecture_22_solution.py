...
def era_year(date):
    ...
    return date[-3:], float(date[:-3]) # pack
...
def main():
    ...
    for jedi in swapi_jedi:
        jedi_names_01.append(jedi['name'])
    ...
    order_66_02 = [
        ...
        era_year(j['death_year'])[0] == 'BBY' and
        era_year(j['death_year'])[1] == 19
        ]
    ...
    jedi_species = [
        (p['name'], 'Human')
        if p['species'][0] == human['url']
        else (p['name'], 'Non-human')
        for p in swapi_jedi
    ]
    ...
    yoda_02 = {key: swapi_yoda[key] for key in swapi_yoda.keys() if key in filter_keys}
